...
class CocoSceneGraphDataset(Dataset):
    def __init__(self, image_dir, instances_json, stuff_json=None,
                 ...
                 filter_mode='LostGAN', mode='train', dump_bbox_dict=None):
        """
        ...
        self.set_image_size(image_size, image_mean=image_mean, image_std=image_std)
        ...
        for image_id in self.image_ids:
            ...
            if min_objects_per_image <= num_objs <= max_objects_per_image:
                new_image_ids.append(image_id)
        self.image_ids = new_image_ids
        if self.filter_mode != 'LostGAN' and self.mode == 'test':
    ...
    def set_image_size(self, image_size, image_mean=[0.5, 0.5, 0.5], image_std=[0.5, 0.5, 0.5]):
        print('called set_image_size', image_size)
        transform = [Resize(image_size), T.ToTensor()]
        if self.normalize_images:
            transform.append(imagenet_preprocess(IMAGENET_MEAN=image_mean, IMAGENET_STD=image_std))
    ...
    def __getitem__(self, index):
        ...
        if self.use_pred_bbox:
            boxes = np.array(self.dump_bbox_dict[os.path.splitext(filename)[0]], dtype=np.float)
            boxes = boxes[0]
...
def imagenet_preprocess(IMAGENET_MEAN=[0.5, 0.5, 0.5], IMAGENET_STD=[0.5, 0.5, 0.5]):
...
def imagenet_deprocess(rescale_image=True, IMAGENET_MEAN=[0.5, 0.5, 0.5], IMAGENET_STD=[0.5, 0.5, 0.5]):
    INV_IMAGENET_MEAN = [-m for m in IMAGENET_MEAN]
    INV_IMAGENET_STD = [1.0 / s for s in IMAGENET_STD]
    transforms = [
